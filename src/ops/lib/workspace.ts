...
export function getWorkspaceScope(role: LocalAuthRole | null): WorkspaceScope {
...
export function filterTasksByRole(role: LocalAuthRole | null, tasks: Task[]): Task[] {
...
export function filterBlockersByRole(role: LocalAuthRole | null, blockers: Blocker[]): Blocker[] {
  const scope = getWorkspaceScope(role);
...
export function filterHandoversByRole(role: LocalAuthRole | null, handovers: Handover[]): Handover[] {
  const scope = getWorkspaceScope(role);
...
export function filterCampaignsByRole(role: LocalAuthRole | null, campaigns: Campaign[]): Campaign[] {
  const scope = getWorkspaceScope(role);
...
export function filterInfluencersByRole(role: LocalAuthRole | null, influencers: CampaignInfluencer[]): CampaignInfluencer[] {
  const scope = getWorkspaceScope(role);
...
export function filterOwnerOptionsByRole(role: LocalAuthRole | null, owners: string[]): string[] {
  const scope = getWorkspaceScope(role);
  if (scope === 'all') return owners;
  return owners.filter((owner) => (scope === 'community' ? isCommunityText(owner) : !isCommunityText(owner)));
}

export function filterTeamOptionsByRole(role: LocalAuthRole | null, teams: string[]): string[] {
  const scope = getWorkspaceScope(role);
  if (scope === 'all') return teams;
  return teams.filter((team) => (scope === 'community' ? isCommunityText(team) : !isCommunityText(team)));
}

export function getWorkspaceLabel(role: LocalAuthRole | null): string {
  const scope = getWorkspaceScope(role);
